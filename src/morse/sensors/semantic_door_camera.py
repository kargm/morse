...
class SemanticDoorCamera(morse.sensors.camera.Camera):
    """
    This sensor emulates a hight level camera that outputs the names of
    the doors that are located within the field of view of the camera as
    well as their state (open/closed).
    ...
    _name = "Semantic door camera"
    _short_desc = "A smart camera allowing to retrieve doors in its \
    field of view"
    ...
    def __init__(self, obj, parent=None):
        ...
        if self.noocclusion:
            logger.info("Semantic door camera running in 'no occlusion' mode (fast mode).")
    ...
    def default_action(self):
        ...
        Iterate over all the tracked doors, and check if they are
        visible for the robot.  Visible doors must have a bounding box
    ...
    def _check_visible(self, obj):
        ...
        bb = blenderapi.persistantstorage().trackedDoors[obj]
        pos = obj.position
        bbox = [[bb_corner[i] + pos[i] for i in range(3)] for bb_corner in bb]

        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("\n--- NEW TEST ---")
            logger.debug("DOOR '{0}' AT {1}".format(obj, pos))
